<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex">
    ...
    <div class="flex-1 ml-20">
      ...
      <section class="relative bg-gradient-to-br from-green-600 via-green-700 to-green-800 py-16 md:py-20 overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-6 relative">
          ...
          <h1 class="text-4xl md:text-5xl font-black text-white mb-3 tracking-tight">Dashboard Pengaduan</h1>
          <p class="text-green-100 text-lg">Kelola dan tangani pengaduan dari warga desa</p>
      ...
      <div class="max-w-7xl mx-auto px-6 -mt-16 relative z-10">
        <div class="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-2">Cari Pengaduan</label>
            ...
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-2">Filter Kategori</label>
              <select v-model="selectedKategori" @change="fetchPengaduan"
                      class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all bg-white">
      ...
      <div class="max-w-7xl mx-auto px-6 py-8">
        ...
        <div v-else>
          <div v-if="pengaduanList.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div v-for="pengaduan in pengaduanList" :key="pengaduan.id" @click="openDetail(pengaduan)"
              class="group bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 cursor-pointer overflow-hidden border-2 border-gray-100 hover:border-green-200 hover:-translate-y-1 flex flex-col">
            
            <div class="p-6 flex-1 flex flex-col">
              <div class="flex items-start justify-between mb-3">
                <span :class="['px-3 py-1 rounded-lg text-xs font-bold', getKategoriColor(pengaduan.kategori)]">
                  {{ pengaduan.kategori }}
                </span>
                <span :class="['px-3 py-1 rounded-lg text-xs font-bold', getStatusColor(pengaduan.status)]">
                  {{ pengaduan.status.toUpperCase() }}
                </span>
              </div>

              <h3 class="text-lg font-bold text-gray-900 mb-3 line-clamp-2 group-hover:text-green-600 transition-colors">
              ...
              <div class="space-y-2 text-sm text-gray-600 mb-3">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span class="font-semibold text-xs">RT {{ pengaduan.rt }} - No. {{ pengaduan.nomor_rumah }}</span>
                </div>
              </div>

              <p class="text-gray-600 text-sm line-clamp-2 flex-1">{{ pengaduan.detail_permasalahan }}</p>
            </div>

            <div class="bg-gradient-to-r from-green-500 to-green-600 px-6 py-3 flex items-center justify-center gap-2 text-white font-bold text-sm group-hover:from-green-600 group-hover:to-green-700 transition-all mt-auto">
    ...
    <div v-if="showModal" @click="closeModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
      <div @click.stop class="bg-white rounded-3xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto">
        <div class="bg-gradient-to-r from-green-500 to-green-600 px-8 py-6 flex items-center justify-between rounded-t-3xl">
          <div>
            <h2 class="text-2xl font-black text-white">Detail Pengaduan</h2>
        ...
        <div class="p-8 space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div class="bg-gradient-to-br from-green-50 to-blue-50 rounded-2xl p-6 border-2 border-green-100">
              <p class="text-sm text-green-600 font-bold mb-3">DATA PELAPOR</p>
              <p class="text-xl font-bold text-gray-900 mb-2">{{ selectedPengaduan.nama_pengirim }}</p>
            ...
            <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 border-2 border-purple-100">
              <p class="text-sm text-purple-600 font-bold mb-3">KATEGORI & STATUS</p>
              <div class="flex flex-wrap gap-3 mb-3">
                <span :class="['px-4 py-2 rounded-xl text-xs font-bold', getKategoriColor(selectedPengaduan.kategori)]">
                  {{ selectedPengaduan.kategori }}
                </span>
                <span :class="['px-4 py-2 rounded-xl text-xs font-bold', getStatusColor(selectedPengaduan.status)]">
          ...
          <div>
            <label class="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">Judul Permasalahan</label>
            <div class="bg-gray-50 rounded-2xl p-5 border-2 border-gray-100">
              <p class="text-gray-900 font-bold text-lg">{{ selectedPengaduan.judul_permasalahan }}</p>
            </div>
          </div>

          <div>
            <label class="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">Detail Permasalahan</label>
            <div class="bg-gray-50 rounded-2xl p-5 border-2 border-gray-100">
          ...
          <div v-if="selectedPengaduan.foto_1 || selectedPengaduan.foto_2">
            <label class="text-sm font-bold text-gray-700 mb-4 flex items-center gap-2">Foto Bukti</label>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div v-if="selectedPengaduan.foto_1" class="rounded-2xl overflow-hidden shadow-xl border-2 border-gray-100">
                <img :src="`/${selectedPengaduan.foto_1}`" alt="Foto 1" class="w-full h-72 object-cover" />
              </div>
              <div v-if="selectedPengaduan.foto_2" class="rounded-2xl overflow-hidden shadow-xl border-2 border-gray-100">
                <img :src="`/${selectedPengaduan.foto_2}`" alt="Foto 2" class="w-full h-72 object-cover" />
              </div>
            </div>
          </div>

          <div class="border-t-2 border-gray-200 pt-8 mt-8">
            <h3 class="text-2xl font-black text-gray-900 mb-6">Update Status & Tanggapan</h3>
            ...
            <div class="space-y-5">
              ...
              <button @click="updatePengaduan" :disabled="updating"
                      class="w-full bg-gradient-to-r from-green-500 to-green-600 text-white py-4 rounded-xl font-bold text-lg hover:shadow-lg hover:shadow-green-500/30 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3">
